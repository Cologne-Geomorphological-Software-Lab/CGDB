...
def _subprocess_side_effect(*, dirty: bool = False, service_status: str = "active"):
...
class DeployCommandTests(TestCase):
    ...
    def test_dry_run_executes_no_effectful_step(self):
        ...
        mock_input.assert_not_called()
        mock_call_command.assert_not_called()
        # The working-tree check is read-only and still runs; nothing else should.
        subprocess_run.assert_called_once()
        assert subprocess_run.call_args.args[0][:2] == ["git", "status"]
        stdout = stdout_io.getvalue()
        assert "Would run: git pull --ff-only" in stdout
